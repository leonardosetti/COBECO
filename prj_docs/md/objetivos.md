...
O **COBECO (Cotação de Bens de Consumo)** é uma aplicação web *desktop-first* que tem como objetivo geral permitir que usuários criem listas de compras personalizadas e, a partir de um catálogo de preços previamente populado (via *seed*), realizem comparações de preços entre diferentes fornecedores, obtendo orçamentos consolidados com base na disponibilidade real de produtos por fornecedor.
...
O sistema será desenvolvido estritamente em formato de **MVP (Minimal Viable Product)** , com dados de fornecedores, produtos e preços previamente populados (via *seed*), permitindo validação rápida da lógica de negócio e da experiência do usuário dentro do prazo estabelecido.
...
| **Seed de Dados** | População inicial de fornecedores, categorias, produtos e preços via scripts de *seed* (dados mockados) |
...
| **RNF02** | Toda comunicação entre UI e backend deve ocorrer via **API RESTful**, regida por contratos **OpenAPI (Swagger)**, adotando a abordagem *API-First* (**SDD - Specification-Driven Development**). |
| **RNF03** | O banco de dados relacional (**PostgreSQL**) deve garantir as propriedades **ACID**, utilizando *migrations* versionadas para controle de schema. |
...
| **RNF08** | O sistema deve implementar **CI/CD** básico (ex: GitHub Actions) para execução automática de testes e validação de qualidade a cada *push*. |
...
|-------------------|--------------|-------------------|----------------|-----------------|
| **Grupo 100%** (9/9) | Supermercado A, Supermercado D, Mercado I | Leite, Pão, Manteiga, Arroz, Feijão, Óleo, Açúcar, Café, Sabonete | Nenhum | **R$ 142,50** ⭐ *Melhor* |
